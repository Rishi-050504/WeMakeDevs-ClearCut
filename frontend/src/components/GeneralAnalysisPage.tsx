...
import { 
  Download, Eye, AlertTriangle, MessageSquare, Menu, ArrowLeft,
  FileText, CheckCircle, Loader2, Lightbulb, Star, Calendar
} from 'lucide-react';
import { Button, Card, CardContent, Textarea, Dialog, DialogContent, DialogHeader, DialogTitle } from './ui/all-components';
...
export function GeneralAnalysisPage({ onLogout, onBack, onViewAnalytics }: GeneralAnalysisPageProps) {
  ...
  const handleSendMessage = async () => { /* ... same as other pages ... */ };
  const handleDownload = () => { /* ... same as other pages ... */ };

  if (isLoading) { return <div className="flex h-screen w-full items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-purple-600" /><p className="ml-4 text-lg">Loading Analysis...</p></div>; }
  if (error || !data) { return <div className="flex h-screen w-full items-center justify-center text-center"><div><AlertTriangle className="h-12 w-12 text-red-500 mx-auto mb-4" /><h2 className="text-xl font-bold">Error</h2><p className="text-red-600 mb-4">{error}</p><Button onClick={onBack}>Go Back</Button></div></div>; }
  ...
  return (
    <div className="min-h-screen w-full bg-white overflow-hidden">
        ...
        <nav className="flex items-center justify-between px-4 md:px-8 py-2 border-b border-black/10 bg-white">
          ...
          <div className="flex items-center space-x-3"><div className="p-2 bg-purple-100 rounded-lg"><FileText className="w-6 h-6 text-purple-600" /></div><div><h1 className="text-xl font-black">General Analysis</h1></div></div>
          <div><Button variant="outline" onClick={() => setShowDocument(true)}>View Document</Button><Button variant="outline" onClick={handleDownload} className="ml-2">Download Report</Button></div>
        </nav>
        ...
        <div className="flex">
            ...
            <div className="w-80 bg-white border-r p-6">
                <div className="text-center mb-8">
                    ...
                    <p className="text-3xl font-black">{data.analysis.riskScore}</p>
                </div>
                {tabItems.map(tab => (
                    <motion.button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center space-x-3 p-4 rounded-lg border my-2 ${activeTab === tab.id ? 'bg-purple-50' : ''}`}>
            ...
            <div className="flex-1 p-4">
              {activeTab === 'keypoints' && (
                <div>
                  ...
                  {data.analysis.keyClauses.map((point, i) => <Card key={i} className="p-4 mb-2">{point}</Card>)}
                </div>
              )}
              {activeTab === 'timeline' && (
                <div>
                  <h2 className="text-2xl font-bold mb-4">Timeline</h2>
                  {(data.mcpResults.timeline || []).map((item, i) => <Card key={i} className="p-4 mb-2"><strong>{item.date}:</strong> {item.event}</Card>)}
                </div>
              )}
            </div>
            ...
            <div className="w-96 bg-white border-l p-6 flex flex-col">
                <h3 className="font-bold mb-4">Document Assistant</h3>
                <div className="flex-1 overflow-y-auto mb-4"></div>
                <div className="flex gap-2"><Textarea /><Button>Send</Button></div>
            </div>
        </div>
