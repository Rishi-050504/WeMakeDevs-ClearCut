...
export function LegalAnalysisPage({ onLogout, onBack, onViewAnalytics }: LegalAnalysisPageProps) {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      if (!documentId) {
        setError("No document ID found. Please upload again.");
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setError(response.data.message || "Failed to retrieve document.");
        }
      } catch (err) {
  ...
  const getRiskColorDetails = (score: number) => {
  ...
  const riskColor = getRiskColorDetails(data.analysis.riskScore);
  ...
  return (
    <div className="min-h-screen w-full bg-white overflow-hidden">
      ...
      <nav className="flex items-center justify-between px-4 md:px-8 py-2 border-b border-black/10 bg-white">
        <div className="flex items-center space-x-4">
          <Button variant="ghost" size="sm" onClick={onBack} className="text-black hover:text-black hover:bg-black/5"><ArrowLeft className="w-4 h-4 mr-2" />Back to Upload</Button>
          <div className="h-6 w-px bg-black/20" />
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-blue-100 rounded-lg"><Scale className="w-6 h-6 text-blue-600" /></div>
            <div><h1 className="text-xl font-black text-black">Legal Analysis</h1><p className="text-xs text-black/60">Contract review & risk assessment</p></div>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <div className="relative">
            ...
            {showMenu && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="absolute right-0 top-full mt-2 w-48 bg-white border border-black/10 rounded-lg shadow-xl z-50"><button onClick={() => { onViewAnalytics(); setShowMenu(false); }} className="w-full px-4 py-3 text-left text-sm hover:bg-black/5 transition-colors">Analytics</button><button onClick={() => { onLogout(); setShowMenu(false); }} className="w-full px-4 py-3 text-left text-sm text-red-600 hover:bg-red-50 transition-colors border-t border-black/10">Logout</button></motion.div>)}
          </div>
        </div>
      </nav>
      ...
      <div className="flex">
        ...
        <div className="w-80 bg-white border-r border-black/10 p-6 max-h-[calc(100vh-64px)] overflow-y-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 text-center">
            <div className="relative w-24 h-24 mx-auto mb-4">
              <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-black/10" /><circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={`${(data.analysis.riskScore / 100) * 251.2} 251.2`} className="text-blue-500" /></svg>
              <div className="absolute inset-0 flex items-center justify-center"><div className="text-center"><div className="text-2xl font-black text-black">{data.analysis.riskScore}</div><div className="text-xs text-black/60">/ 100</div></div></div>
            </div>
            <h3 className="font-bold text-black mb-1">Risk Score</h3>
            <Badge variant="secondary" className={`${riskColor.bg} ${riskColor.text} ${riskColor.border}`}>{riskColor.label}</Badge>
          </motion.div>
          {/* Navigation Tabs, Contract Info... */}
        </div>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'overview' && (
                    <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4">
                        <h2 className="text-xl font-black text-black">Contract Overview</h2>
                        <Card><CardContent className="p-4"><h3 className="font-bold mb-2">Contract Parties</h3>{(data.mcpResults.entities?.organizations || []).map((party, i) => <p key={i}>{party}</p>)}</CardContent></Card>
                        <div className="grid grid-cols-4 gap-4">
                           <Card><CardHeader><CardTitle>Total Clauses</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{data.analysis.keyClauses.length}</p></CardContent></Card>
                           <Card><CardHeader><CardTitle>Risk Level</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{riskColor.label}</p></CardContent></Card>
                ...
                {activeTab === 'risks' && (
                     <motion.div key="risks" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                        <h2 className="text-xl font-black">Risk Analysis</h2>
                        {data.analysis.keyClauses.map((clause, i) => <Card key={i} className="mt-2"><CardContent className="p-4">{clause}</CardContent></Card>)}
                     </motion.div>
                )}
                {activeTab === 'recommendations' && (
                     <motion.div key="recommendations" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                        ...
                        {data.analysis.recommendations.map((rec, i) => <Card key={i} className="mt-2"><CardContent className="p-4">{rec}</CardContent></Card>)}
                     </motion.div>
                )}
            </AnimatePresence>
        </div>
        ...
        {/* Chat Sidebar */}
        <div className="w-96 bg-white border-l border-black/10 p-6 max-h-[calc(100vh-64px)] flex flex-col">
            <h3 className="font-bold text-black mb-4">Legal Assistant</h3>
            <div className="flex-1 space-y-4 overflow-y-auto mb-4">
                {chatHistory.map((msg, i) => <div key={i} className={`p-2 rounded ${msg.role === 'user' ? 'bg-blue-100' : 'bg-gray-100'}`}>{msg.message}</div>)}
            </div>
            <div className="flex gap-2"><Textarea value={chatMessage} onChange={e => setChatMessage(e.target.value)} placeholder="Ask about clauses..." /><Button onClick={handleSendMessage}>Send</Button></div>
        </div>
      </div>
      
      <Dialog open={showDocument} onOpenChange={setShowDocument}><DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto"><DialogHeader><DialogTitle>{data.fileName}</DialogTitle></DialogHeader><div className="p-6 bg-gray-100 rounded-lg whitespace-pre-wrap font-mono text-sm">{data.rawText}</div></DialogContent></Dialog>
    </div>
  );
}