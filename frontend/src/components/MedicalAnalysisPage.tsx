...
interface AnalysisData {
  ...
  analysis: {
    riskScore: number;
    keyClauses: string[]; // findings
    obligations: string[]; // follow-ups
...
export function MedicalAnalysisPage({ onLogout, onBack, onViewAnalytics }: MedicalAnalysisPageProps) {
  ...
  // Rest of the component JSX, adapted to use `data` state
  const tabItems = [
    { id: 'overview', label: 'Overview', icon: Activity },
    { id: 'findings', label: 'Findings', icon: AlertTriangle },
    { id: 'recommendations', label: 'Recommendations', icon: CheckCircle },
  ];
  const riskLabel = data.analysis.riskScore > 66 ? 'High Urgency' : data.analysis.riskScore > 33 ? 'Moderate Urgency' : 'Low Urgency';
  ...
  return (
     <div className="min-h-screen w-full bg-white overflow-hidden">
        ...
        <nav className="flex items-center justify-between px-4 md:px-8 py-2 border-b border-black/10 bg-white">
          ...
          <div className="flex items-center space-x-3"><div className="p-2 bg-red-100 rounded-lg"><Heart className="w-6 h-6 text-red-600" /></div><div><h1 className="text-xl font-black">Medical Analysis</h1></div></div>
        ...
        <div className="flex">
            ...
            <div className="w-64 bg-white border-r p-4">
                <div className="text-center mb-5">
                    <h3 className="font-bold">Urgency Score</h3>
                    <p className="text-2xl font-black">{data.analysis.riskScore}</p>
                    <Badge>{riskLabel}</Badge>
                </div>
                {tabItems.map(tab => (
                    <motion.button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center space-x-2 p-3 rounded-lg border ${activeTab === tab.id ? 'bg-red-50' : ''}`}>
            ...
            <div className="flex-1 p-3">
                {activeTab === 'findings' && <div><h2 className="font-bold text-lg">Key Findings</h2>{data.analysis.keyClauses.map((f, i) => <Card key={i} className="p-2 mt-2">{f}</Card>)}</div>}
                {activeTab === 'recommendations' && <div><h2 className="font-bold text-lg">Recommendations</h2>{data.analysis.recommendations.map((r, i) => <Card key={i} className="p-2 mt-2">{r}</Card>)}</div>}
            </div>

            {/* Chat */}
            <div className="w-64 bg-white border-l p-3 flex flex-col"><h3 className="font-bold mb-3">Medical Assistant</h3><div className="flex-1 overflow-y-auto space-y-2"></div><div className="flex gap-1 mt-2"><Textarea /><Button>Send</Button></div></div>
        </div>
